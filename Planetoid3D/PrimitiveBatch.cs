...
namespace Planetoid3D
{
    ...
    public class PrimitiveBatch : IDisposable
    {
        ...
        public PrimitiveBatch(GraphicsDevice graphicsDevice)
        {
            ...
            //    VertexPositionColor.VertexElements); NO LONGER NEEDED IN XNA 4.0
        ...
        public void Begin(PrimitiveType primitiveType)
        {
            ...
            //device.VertexDeclaration = vertexDeclaration; XNA 4.0
            //basicEffect.Begin();
        ...
        //Draw the line
        /// <summary>
        /// Draw a line from p1 to p2 with precision step and color col
        /// </summary>
        ...
        public void DrawLineWidth(Vector2 p1, Vector2 p2, Color color, int width)
        {
            float dir = (float)Math.Atan2(p2.Y - p1.Y, p2.X - p1.X);
            dir += (float)(Math.PI / 2);
            for (float a = 0; a < width; a += 0.5f)
            {
                AddVertex(
                    new Vector2(
                    p1.X + (float)(Math.Cos(dir) * a),
                    p1.Y + (float)(Math.Sin(dir) * a))
                    , color);
                AddVertex(
                    new Vector2(
                    p2.X + (float)(Math.Cos(dir) * a),
                    p2.Y + (float)(Math.Sin(dir) * a))
                    , color);

            }
        }*/

        /// <param name="zone">The zone, specify by a rectangle, in which you want to draw the rectangle</param>
        /// <param name="color">Color of the rectangle</param>
        /// <param name="filled">Specify if the rectangle must be filled or not</param>
        /// <param name="depth">Depth of the rectangle</param>
        /*public void DrawRectangle(Rectangle zone, Color color, bool filled)
        {
            //graphics.GraphicsDevice.RenderState.DepthBufferWriteEnable = false;
            if (filled)
            {
                for (float a = 0; a < zone.Height; a += 0.1f)
                {
                    AddVertex(new Vector2(zone.Left + 1, zone.Top + a), color);
                    AddVertex(new Vector2(zone.Right - 1, zone.Top + a), color);
                }
            }
            else
            {
                AddVertex(new Vector2(zone.Left, zone.Top), color);
                AddVertex(new Vector2(zone.Right, zone.Top), color);
                AddVertex(new Vector2(zone.Right, zone.Top), color);
                AddVertex(new Vector2(zone.Right, zone.Bottom), color);
                AddVertex(new Vector2(zone.Right, zone.Bottom), color);
                AddVertex(new Vector2(zone.Left, zone.Bottom), color);
                AddVertex(new Vector2(zone.Left, zone.Bottom), color);
                AddVertex(new Vector2(zone.Left, zone.Top), color);
            }
        }
        */
        /// <summary>Draw a circle with center center and radius radius, color col1, you can specify if it is filled and the precision of the drawing</summary>
        /// <param name="center">The center of the circle</param>
        /// <param name="radius">The radius of the circle</param>
        /// <param name="col">The color of the circle</param>
        /// <param name="precision">Specify the precision of the drawing, 36 default</param>
        public void DrawCircle(Vector3 center, float radius, Color col, int precision)
        {
            if (precision <= 0 || radius <= 0)
            {
                AddVertex(center, col);
                return;//exit state, the circle is not drawable
            }
            for (int a = 0; a <= 360; a += (360 / precision))
            {
                AddVertex(center + new Vector3(
                    (float)(Math.Sin(MathHelper.ToRadians(a)) * radius),
                     0.0f,
                    (float)(Math.Cos(MathHelper.ToRadians(a)) * radius))
                    , col);
            }
        }
        #endregion
