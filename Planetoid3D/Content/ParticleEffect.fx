...
VertexShaderOutput ParticleVertexShader(VertexShaderInput input)
{
	...
	if (dot(CamForward,normalize(input.Position-CamPosition))<0.1)
	{
		size=0;
	}
	else
	{
		size = ComputeParticleSize(input.Random.y, normalizedAge) * input.Size;
	}
    output.Position.xy += mul(input.Corner, rotation) * size * ViewportScale;
    
    output.Color = ComputeParticleColor(output.Position, input.Random.z, normalizedAge)*input.Color;
